...
function displayFieldDetails(fieldKey) {
    ...
    const visibleTrailers = (field.trailers || [])
        .map((trailer, index) => ({ ...trailer, originalIndex: index })) // Keep original index
        .filter(trailer => 
            (trailer.empty !== null && trailer.empty > 0) || // Finalized trailers are visible to everyone
            trailer.fullBy === currentUser.uid                 // Unfinalized are visible only to their creator
        );
...
function showWeightModal(mode, index = -1) {
    ...
    const content = `
        ...
        <input type="number" inputmode="numeric" id="weight-input" data-mode="${mode}" data-index="${index}" class="w-full p-4 border-2 border-gray-300 rounded-lg text-xl text-center focus:ring-2 focus:ring-green-500 transition" placeholder="0 kg">
